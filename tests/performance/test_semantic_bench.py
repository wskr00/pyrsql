...
def test_query_semantic_analysis_remains_fast() -> None:
    ...
    analyzer = SemanticAnalyzer(QueryOptions(procedure_whitelist=("upper",)))
    elapsed = timeit(lambda: analyzer.analyze(expression), number=5000)
...
def test_sort_semantic_analysis_remains_fast() -> None:
    ...
    analyzer = SortAnalyzer(SortOptions(procedure_whitelist=("upper",)))
    elapsed = timeit(lambda: analyzer.analyze(fields), number=5000)
    average_microseconds = elapsed / 5000 * 1_000_000
    assert average_microseconds < 200.0
